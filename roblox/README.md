...
接続確認 ✅ 完了。`src/ServerScriptService/GameSetup.server.lua` に、雑草ぬきゲームの最小ロジック
(雑草に近づいてぬくとポイント+10、おはなをぬくとポイント-15、画面右上のプレイヤーリストに
ポイントが表示される)を追加済みです。

`rojo serve` を起動してStudioと接続したままにしておけば、このリポジトリを `git pull` するだけで
最新のスクリプトが自動的にStudio側へ同期されます(再接続は不要です)。次にStudioで **▶(再生)** を
押すと、雑草・おはなに近づいた時に「ぬく」ボタン(ProximityPrompt)が出るはずです。
