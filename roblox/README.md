...
DataStore(進捗保存)はデフォルトではStudio上のテストプレイで使えません。以下の設定が必要です。

1. Studio上部の **ホーム** タブ → **ゲーム設定**
2. 左メニューの **セキュリティ** タブを開く
3. **「Studio内でのAPIサービスへのアクセスを有効にする」** をONにする
4. 保存して閉じる

これをしないと、保存・読込が失敗し続けます(エラーは出力ウィンドウに`warn`として表示されます)。
